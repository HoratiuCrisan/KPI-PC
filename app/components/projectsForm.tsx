...
const ProjectsForm = () => {
    ...
    const handleSubmit = (event: any) => {
        ...
        {/* getting the day-month-year from our date object */}
        const startDate = projectDate[0].toString().split(" ")
        const endDate = projectDate[1].toString().split(" ")

        console.log(assignedUsers)

        const userListObject = assignedUsers.map((user) => {
            return {
                displayName: user.label
            }
        })
        {/* Createing an object named project with the properties from the fields to store in the db */}
        const project = {
           projectName,
           projectTeamName,
           projectDetails,
